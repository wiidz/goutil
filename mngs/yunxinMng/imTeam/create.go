...
type CreateParam struct {
	TeamName           string `json:"tname"`                     // 群名称，最大长度64字符
	Owner              string `json:"owner"`                     // 群主用户帐号，最大长度32字符
	Members            string `json:"members"`                   // 邀请的群成员列表。["aaa","bbb"](JSONArray对应的accid，如果解析出错会报414)，members与owner总和上限为200。members中无需再加owner自己的账号。
	Announcement       string `json:"announcement,omitempty"`    // 群公告，最大长度1024字符
	Intro              string `json:"intro,omitempty"`           // 群描述，最大长度512字符
	Msg                string `json:"msg"`                       // 邀请发送的文字，最大长度150字符
	ManagerAgree       int    `json:"magree"`                    // 管理后台建群时，0不需要被邀请人同意加入群，1需要被邀请人同意才可以加入群。其它会返回414
	JoinMode           int    `json:"joinmode"`                  // 群建好后，sdk操作时，0不用验证，1需要验证,2不允许任何人加入。其它返回414
