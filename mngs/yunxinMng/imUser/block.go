...
type BlockParam struct {
	...
	NeedKick      string `json:"needkick"`                  // 	是否踢掉被禁用户，true或false，默认false
	KickNotifyExt string `json:"kickNotifyExt"`             // 	该操作的扩展字段，可透传至客户端SDK（版本至少为v7.7.0）
}
...
func (api *Api) Block(param *BlockParam) (*imClient.CommonResp, error) {
	res, err := api.Client.Post("/user/block.action", param, &imClient.CommonResp{})
