...
func (mng *IdentityMng) Login(_ context.Context, loginID string, device string) (TokenPair, error) {
    if device == "" {
        if mng.defaultDevice != "" { device = mng.defaultDevice } else { device = "client" }
    }
...
func (mng *IdentityMng) LogoutByLoginID(_ context.Context, loginID string) error { return stputil.Logout(loginID) }
